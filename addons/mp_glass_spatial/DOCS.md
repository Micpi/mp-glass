# Installer MP Nexus Spatial

**Le plus simple avec Gemini : ne pas installer cet add-on.** Depuis MP Nexus 0.2.1, une clé API dans les options de l’intégration suffit. [Parcours en trois étapes](../../docs/GEMINI_QUICKSTART.md).

Les instructions ci-dessous concernent uniquement le mode avancé avec conversion locale. Dans les options MP Nexus, choisir **Add-on Spatial — avancé** pour afficher les champs adresse et clé de liaison.

L’add-on traite les documents et appelle Gemini. L’import, la correction et la consultation se trouvent dans MP Nexus Studio, sans interface Ingress séparée ni accès du worker à l’API HA.
...
2. Paramètres → Modules complémentaires / Apps → Boutique → rechercher les mises à jour, puis installer **MP Nexus Spatial** dans les add-ons locaux.
...
5. Choisir `api_token`, clé de liaison aléatoire d’au moins 24 caractères, distincte de la clé Gemini. La recopier dans les options de l’intégration MP Nexus.
6. Démarrer. Garder le port hôte vide : l’adresse interne d’une installation locale est `http://local-mp-glass-spatial:8099`.
7. Paramètres → Appareils et services → MP Nexus → Configurer : renseigner cette adresse et la clé de liaison.
8. Ouvrir MP Nexus Studio → Plan 3D.
