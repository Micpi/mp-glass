...
if(spatial) {
  ...
  rooms[0]!.entityIds=['light.circuit_0','sensor.salon_temperature','sensor.salon_humidity'];
...
const hass: Hass = { connection:{},states:snapshot.states,language:'fr',user:{id:'demo',is_admin:true},callWS:async<T>()=>[] as T,callService:async(domain,service,data)=>{
  ...
  for(const entityId of ([] as unknown[]).concat(data.entity_id).map(String)){const old=states[entityId]!;states[entityId]={...old,state:service==='turn_off'?'off':'on',attributes:{...old.attributes,...('brightness_pct' in data?{brightness:Number(data.brightness_pct)*255/100}:{})}};}
  hass.states=states;
