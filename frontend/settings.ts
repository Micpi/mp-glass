...
export class MPGlassSettings extends LitElement {
  ...
  render() {
    ...
    return html`<main aria-busy=${this.busy}><div class="page-head"><div><p class="overline">MP GLASS · INTEGRATOR STUDIO</p><h1>${t(lang,'setup')}</h1><p>${t(lang,'intro')}</p></div>${project ? html`<nav><button ?disabled=${this.busy} @click=${this.scan}>${mpIcon('scan',18)} ${t(lang,'scan')}</button><button class="primary" ?disabled=${this.busy || !this.hass?.user?.is_admin} @click=${this.save}>${mpIcon('check',18)} ${t(lang,'save')}</button></nav>` : nothing}</div>
      ...
      ${project ? html`<div class="editor"><section class="surface"><div class="panel-title">${mpIcon('palette',24)}<h2>Identité et apparence</h2></div><div class="controls">
        ...
        <label>${t(lang,'preset')}<select .value=${a!.preset} @change=${(e:Event)=>this.appearance('preset',(e.target as HTMLSelectElement).value as Preset)}>${['glass-blue','glass-warm','glass-dark','glass-light','glass-oled','glass-neutral'].map(p=>html`<option value=${p}>${p.replace('glass-', 'Glass ')}</option>`)}</select></label>
        ...
        <label class="wide"><span class="range-head"><span>Assombrissement du fond</span><strong>${Math.round((a!.backgroundDim ?? .44)*100)} %</strong></span><input type="range" min="0" max="80" .value=${String((a!.backgroundDim ?? .44)*100)} @input=${(e:Event)=>this.appearance('backgroundDim',Number((e.target as HTMLInputElement).value)/100)}></label>
        ...
        <label class="wide">Accroche<input maxlength="100" .value=${a!.eyebrow ?? ''} @input=${(e:Event)=>this.appearance('eyebrow',(e.target as HTMLInputElement).value)}></label>
