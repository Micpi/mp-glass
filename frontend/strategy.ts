...
/** Codes answered while the MP Nexus integration is still starting or reloading. */
...
async function readProjectWhenReady(hass: Hass) {
  for (let attempt = 1; ; attempt++) {
    try { return await readProject(hass); }
      ...
      if (attempt === 10) throw new Error(tr('MP Nexus n’est pas chargé dans Home Assistant. Vérifiez l’intégration dans Paramètres → Appareils et services, puis rechargez la page.'));
...
export class MPGlassStrategy extends HTMLElement {
  ...
  static async generate(config: { debug?: boolean }, hass: Hass) {
    generated = true;
    followHass(hass);
    const [snapshot, {project, version}] = await Promise.all([HARegistryReader.read(hass), readProjectWhenReady(hass)]);
    return MPDashboardComposer.compose(MPDiscoveryEngine.discover(snapshot, project), project, !!config.debug && !!hass.user?.is_admin, tr('Inventaire'), { lights: tr('Lumières'), rooms: tr('Pièces') }, version);
  }
}
