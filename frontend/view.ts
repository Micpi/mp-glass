...
export class MPGlassView extends LitElement {
  ...
  static styles = css`
    ...
    *{box-sizing:border-box}.backdrop,.shade,.ambient{position:fixed;inset:0;pointer-events:none}.backdrop{z-index:-4;background-size:cover;background-repeat:no-repeat;transform:scale(1.035);filter:blur(var(--mp-bg-blur,0)) saturate(var(--mp-bg-saturation,1))}.shade{z-index:-3;background:linear-gradient(90deg,rgba(2,13,25,.8),rgba(3,18,31,.17) 58%,rgba(2,10,19,.38)),linear-gradient(0deg,rgba(2,11,21,.95),transparent 72%)}.ambient{z-index:-2;background:radial-gradient(circle at 18% 15%,color-mix(in srgb,var(--mp-accent,#69b7ff) 14%,transparent),transparent 34%),radial-gradient(circle at 85% 70%,color-mix(in srgb,var(--mp-secondary,#efbd8b) 10%,transparent),transparent 28%)}
    ...
    .empty{display:flex;align-items:center;flex-wrap:wrap;gap:12px;padding:26px 22px;border-radius:var(--mp-radius);color:#c9d9e8;font-size:14px}.empty>.mp-icon{color:var(--mp-accent)}.empty a{margin-left:auto;min-height:40px;display:inline-flex;align-items:center;gap:8px;padding:0 13px;border-radius:12px;text-decoration:none;color:#eef7ff;border:1px solid rgba(212,232,250,.2);background:rgba(255,255,255,.055);font-size:12px}.empty a:hover{border-color:var(--mp-accent);background:color-mix(in srgb,var(--mp-accent) 15%,transparent)}
  ...
  protected updated() { this.fitNavigation(); this.watchConnection(); }
  /** False while the Home Assistant connection is interrupted: the clock dot and the section status say it. */
  private online = true;
  private connectionWatched?: object;
  /** Follows the real connection events instead of assuming the dashboard is always in sync. */
  private watchConnection() {
    const conn = this.hass?.connection as unknown as { connected?: boolean; addEventListener?: (type: string, listener: () => void) => void } | undefined;
    if (!conn?.addEventListener || this.connectionWatched === conn) return;
    this.connectionWatched = conn;
    const update = () => { this.online = conn.connected !== false; };
    conn.addEventListener('ready', update);
    conn.addEventListener('reconnected', update);
    conn.addEventListener('disconnected', update);
    update();
  }
  ...
  private section(title: string, subtitle: string, icon?: MPIconName) {
    return html`<div class="section-head"><div class="section-label"><span class="section-icon">${mpIcon(icon ?? (this.viewKind==='rooms'?'rooms':'bulb'),23)}</span><div><h2>${title}</h2><p>${subtitle}</p></div></div><span class="section-action" role="status"><i class=${this.online?'':'off'}></i> ${this.online?tr('Synchronisé avec Home Assistant'):tr('Connexion interrompue, reconnexion…')}</span></div>`;
  }
  ...
  private tile(id: string, room: string) {
    ...
    return html`<button class=${`tile glass ${on?'on':''}`} title=${tr('Détails')} aria-label=${tr('Détails de {name}',{name})} @click=${()=>this.moreInfo(id)}>
  ...
  /** An empty page says why, and offers the next step instead of a blank grid. */
  private emptyState() {
    const message = this.viewKind === 'lights' ? tr('Aucune lumière détectée pour le moment.')
      : this.viewKind === 'area' ? tr('Aucun équipement dans cette pièce pour le moment.')
      : tr('Aucun équipement à afficher pour le moment.');
    return html`<p class="empty glass" role="status">${mpIcon('scan',22)}<span>${message}</span>${this.hass?.user?.is_admin ? html`<a class="settings-link" href="/mp-glass-settings">${mpIcon('tune',16)} ${tr('Ouvrir le Studio')}</a>` : nothing}</p>`;
  }
      ...
      <div class="shell" style=${style}>
        <header class="glass">
          ...
          <div class="header-tools">
            ${this.languageMenu()}
            ${a.showClock === false ? nothing : html`<div class="clock"><strong>${new Intl.DateTimeFormat(locale(),{hour:'2-digit',minute:'2-digit'}).format(now)}</strong><small title=${this.online?tr('Connecté à Home Assistant'):tr('Connexion interrompue, reconnexion…')}><i class=${this.online?'':'off'}></i>${new Intl.DateTimeFormat(locale(),{weekday:'short',day:'numeric',month:'short'}).format(now)}</small></div>`}
          </div>
        </header>
        ...
        ${this.viewKind === 'info' ? this.renderInfo() : this.viewKind === 'rooms' ? html`
          ...
          ${this.areas.length || this.inventory ? nothing : html`<p class="empty glass" role="status">${mpIcon('rooms',22)}<span>${tr('Aucune pièce détectée. Créez des zones dans Home Assistant, puis ouvrez le Studio pour analyser votre installation.')}</span>${this.hass?.user?.is_admin ? html`<a class="settings-link" href="/mp-glass-settings">${mpIcon('tune',16)} ${tr('Ouvrir le Studio')}</a>` : nothing}</p>`}
