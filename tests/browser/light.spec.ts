...
test('commands the correct light; dimmer only where declared; more-info bubbles',async({page})=>{
  await page.goto('/');const first=page.locator('mp-glass-light-v3').nth(0);
  ...
  await expect(page.locator('mp-glass-light-v3').nth(1).getByRole('slider')).toHaveCount(0);
...
test('displays unavailable and service errors without claiming success',async({page})=>{
  ...
  const first=page.locator('mp-glass-light-v3').first();await first.getByRole('button',{name:'Allumer',exact:true}).click();await expect(first.getByRole('alert')).toContainText('Action impossible');await expect(first.getByRole('button',{name:'Allumer',exact:true})).toBeEnabled();
...
for(const [name,width,height] of [['phone-portrait',390,844],['phone-landscape',844,390],['tablet-portrait',820,1180],['tablet-landscape',1180,820],['desktop',1440,1000],['wall',1920,1080]] as const){
  test(`${name}: no overflow and visual baseline`,async({page})=>{await page.clock.setFixedTime(new Date('2026-09-12T20:42:00'));await page.setViewportSize({width,height});await page.goto('/');await expect(page.locator('mp-glass-light-v3')).toHaveCount(3);expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);await expect(page).toHaveScreenshot(`${name}.png`,{fullPage:true,animations:'disabled'});});
}
